...
- В компоненте `Image` был удалено свойство `objectFit`. Теперь нужно его вписывать в CSS.
- Исправлена ошибка, из-за которой во время загрузки страницы "слетали" стили на несколько секунд, из-за чего некорректно отображалась страница.
- На главной странице в разделе "CV-резюме" теперь можно скачивать и просматривать файл.
- Исправлена ошибка, из-за которой во время загрузки страницы по неверной ссылки дублировали компоненты `Header` и `Footer`.
- Теперь в компоненте `Description` последняя строка будет центрироваться.
- Исправлена ошибка, из-за которого svg-изображение отображалось некорректно.
...
- Добавлен плагин `react-router-dom` в проект и добавлена папка `routes` для маршрутизации.
- Добавлены две страницы:
  1. `main` - главная страница по адресу `/`;
  2. `project` - страница конкретного проекта по адресу `/projects/<number>`, описывающий все подробности;
  3. `errorBoundary` - страница, которая показывает ошибку, если в URL введён неверный адрес.
- Исправлена ошибка, из-за которой во всех страницах отображался полностью хедер вместо того, чтобы отобразить обычный `Navbar`.
- Добавлен файл `arrayControl.ts`, который будет добавлять список инструментов, языков и библиотек.
- В секции "Портфолио" появилась возможность переходить по проектам, нажимая на кнопку `Подробнее`.
- В компоненте `Navbar` при клике ссылок можно плавно переходить по страницам.
...
- Был удалён файл `spotButtons.ts`, т.к. теперь нет необходимости пользоваться.
